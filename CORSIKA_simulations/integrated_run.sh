...
count_particles_script="/opt/count_particle.py"
...
# The directory where integrated_run.sh is located is in /opt.
...
EOF
    ...
    cd "$corsika_dir"
    if [ ${#time} -eq 3 ]; then
        ./restriccion_polar.sh -d "${theta}-${thetamax}deg_000${time}/" -m -90 -n 90
    elif [ ${#time} -eq 4 ]; then
        ./restriccion_polar.sh -d "${theta}-${thetamax}deg_00${time}/" -m -90 -n 90
    else
    ...
    # Create the results directory in /opt
