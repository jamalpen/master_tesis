...
if [ $# -lt 2 ]; then
    echo "Usage: $0 <theta> <time>"
    exit 1
fi

# Take the arguments as theta and time
theta=$1
time=$2

# Determine the pattern for files and folders based on the length of 'time'
if [ ${#time} -eq 3 ]; then
    file_pattern="go-${theta}-90deg_000${time}-*"
    folder_pattern="${theta}-90deg_000${time}"
elif [ ${#time} -eq 4 ]; then
    file_pattern="go-${theta}-90deg_00${time}-*"
    folder_pattern="${theta}-90deg_00${time}"
else
